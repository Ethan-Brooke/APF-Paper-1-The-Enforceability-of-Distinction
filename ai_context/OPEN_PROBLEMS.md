...
### Paper 6 status
Listed as Project C.1 in Paper 6 v2.0-PLEC Appendix C. `check_Regime_R` (v7.8) formalizes the regime in which PLEC is well-posed but does not fully prove existence + uniqueness in general.
...
### What's stated
The five-type regime-exit taxonomy (Types I–V) is formalized as bank checks in `apf/plec.py` (v7.8). Each type has an executable witness.
