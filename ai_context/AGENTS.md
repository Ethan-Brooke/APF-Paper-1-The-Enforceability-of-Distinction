...
**You are reading the companion to Paper 1** (this repo: https://github.com/Ethan-Brooke/APF-Paper-1-The-Enforceability-of-Distinction).
...
When a user's question is about a result outside Paper 1, point them to the relevant row above. Do not attempt to re-derive cross-paper results from the bundled subset — the canonical state is the codebase ([Zenodo concept DOI](https://doi.org/10.5281/zenodo.18529115)), not this slice.

---

## 1. What this repository is

This is the **paper-companion release repo for Paper 1 of the Admissibility Physics Framework (APF) series**. Specifically: The Enforceability of Distinction: Quantum Structure from Finite Enforcement Capacity.

The repo bundles:

- The manuscript and Technical Supplement (`.tex` + `.pdf`)
- A vendored subset of the APF codebase containing only the `check_*` functions that Paper 1 directly references (`apf/core.py`, `apf/apf_utils.py`, `apf/bank.py`)
...
The framework-wide canonical codebase (440 bank-registered theorems, 25 modules, 457 `verify_all` checks; canonical Phase-18 baseline) is **not** bundled here. Only the Paper 1 subset is runnable locally. See `ai_context/THEOREMS.md` for the full bank listing with bundled-in-this-repo flags.
...
If you're loading this repo cold and want to understand Paper 1 well enough to reason about it:
...
7. **`Paper_1_Enforceability_of_Distinction_v5.2.tex`** or `.pdf` — the manuscript. Read §1 (intro) and the section you care about. Don't try to read the whole paper first.
