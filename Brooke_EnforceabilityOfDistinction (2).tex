...
\textit{Step 1} (Hypothesis).  Suppose $\varepsilon^* = 0$.  Then for every $n \in \mathbb{N}$, there exists an enforceable distinction $d_n$ with $\varepsilon(d_n) < 1/n$.  By M, there exist at least countably many pairwise-admissible distinctions in this low-cost regime (no global obstruction prevents their simultaneous enforcement when each costs $< 1/n$).

\textit{Step 2} (Cheap destruction exists).  Consider any such $d_n$.  Since $\varepsilon(d_n) < 1/n$, the infimum of destructive costs is below $1/n$.  Therefore there exists an admissible perturbation of cost $< 1/n$ that destroys $d_n$.

\textit{Step 3} (Contradiction with robustness).  As $n \to \infty$, the sequence $\{d_n\}$ yields enforceable distinctions destroyable at cost $\to 0$.  But each $d_n$ was assumed enforceable, meaning the universe actively maintains it.  A distinction that can be destroyed at arbitrarily small cost is operationally indistinguishable from ``not enforced at all''---the enforcement mechanism cannot distinguish ``$d$ holds'' from ``$d$ does not hold'' if toggling between them costs less than any positive threshold.  This contradicts the assumption that each $d_n$ is a physically maintained fact.  Contradiction.
...
\textit{Step 2} (Joint defense is harder).  By the Perturbation Model, when $d_1$ and $d_2$ share an enforcement budget, the joint perturbation space $\mathcal{P}(\{d_1, d_2\})$ strictly contains $\mathcal{P}(d_1) \cup \mathcal{P}(d_2)$: there exist combined attacks that simultaneously threaten both distinctions.  Defending against a strictly larger perturbation space costs strictly more than the sum of individual defenses.
...
\textit{Step 2} (Wedderburn--Artin decomposition).  Let $\mathcal{A}$ be the enforcement algebra from Step~1: a finite-dimensional, unital, associative $*$-algebra over $\mathbb{C}$ (unital because the identity operation---enforcing nothing---costs zero capacity and serves as the algebraic identity).  The Jacobson radical $J(\mathcal{A})$ consists of elements $a \in \mathcal{A}$ such that $\omega(a^* a) = 0$ for every state $\omega$.  But OR0 (Faithfulness) requires that distinct enforcement configurations produce distinct states.  A nonzero $a \in J(\mathcal{A})$ would annihilate every state, contradicting faithfulness.  Therefore $J(\mathcal{A}) = \{0\}$ and $\mathcal{A}$ is semisimple.  By the Wedderburn--Artin theorem, every finite-dimensional semisimple associative algebra over $\mathbb{C}$ decomposes uniquely as
\begin{equation}
\mathcal{A} \;\cong\; \bigoplus_{k=1}^K M_{n_k}(\mathbb{C}),
\end{equation}
a direct sum of full matrix algebras.  The involution $*$ coincides with the conjugate-transpose on each factor (since the only positive-definite $*$-structure on $M_n(\mathbb{C})$ is the standard one).  This decomposition is not assumed; it is forced by finiteness (OR3), faithfulness (OR0), and the classification of semisimple algebras.

\textit{Step 3} (GNS representation --- general case).  Each factor $M_{n_k}(\mathbb{C})$ admits a faithful state: define $\omega_k(X) = \mathrm{tr}(X)/n_k$.  This is normalized ($\omega_k(I) = 1$), positive ($\omega_k(X^\dagger X) = \mathrm{tr}(X^\dagger X)/n_k \ge 0$), and faithful ($\omega_k(X^\dagger X) = 0$ $\Rightarrow$ $\|X\|_F = 0$ $\Rightarrow$ $X = 0$).  The GNS construction applied to $\omega_k$ produces a Hilbert space $\mathcal{H}_k$ with inner product $\langle X, Y \rangle_k = \omega_k(X^\dagger Y)$ and a faithful $*$-representation $\pi_k: M_{n_k}(\mathbb{C}) \hookrightarrow B(\mathcal{H}_k)$ via left multiplication.  The full enforcement algebra is then faithfully represented on $\mathcal{H} = \bigoplus_k \mathcal{H}_k$.  Since each $M_{n_k}(\mathbb{C})$ acts naturally on $\mathbb{C}^{n_k}$, we have $\mathcal{A} \hookrightarrow B(\mathcal{H})$ with $\mathcal{H} = \bigoplus_k \mathbb{C}^{n_k}$ and $\dim \mathcal{H} = \sum_k n_k$.

\textit{Step 4} (Why complex?).  The $*$-operation on $\mathcal{A}$ admits three Wedderburn--Artin possibilities for each factor: $M_n(\mathbb{R})$, $M_n(\mathbb{C})$, or $M_n(\mathbb{H})$ (real, complex, or quaternionic matrix algebras).  The real case is excluded by T1: a commuting pair of Hermitian generators in $M_n(\mathbb{R})$ always admits a simultaneous eigenbasis, contradicting the non-commutativity of enforcement.  The quaternionic case is excluded by OR1 (convex mixing): quaternionic quantum mechanics does not admit a tensor product consistent with independent subsystems, violating $L_{\mathrm{loc}}$ at the composite level.  (This is a well-known obstruction: the non-associativity of the quaternionic tensor product prevents a consistent rule for combining independent systems, a result established in the generalized probabilistic theory literature~\cite{Hardy2001}.)  Therefore each factor is $M_{n_k}(\mathbb{C})$ and $\mathcal{H}$ is a complex Hilbert space.

\textit{Step 5} (Dimension bound).  By OR3, $\mathcal{A}$ has at most $N = \lfloor C/\varepsilon^* \rfloor$ generators.  Since $\dim M_{n_k}(\mathbb{C}) = n_k^2$, we have $\sum_k n_k^2 \le N^2$, giving $\dim \mathcal{H} = \sum_k n_k \le N$.  A tight bound requires specifying the representation structure of the enforcement algebra, which depends on the budget deployment (Papers~2--7).

\textit{Illustration.}  For a single binary distinction, take $\mathfrak{E}_{d_1} = \sigma_x$ and $\mathfrak{E}_{d_2} = \sigma_z$.  These generate $M_2(\mathbb{C})$, with GNS Hilbert space $\mathcal{H} \cong \mathbb{C}^2$.  This is the simplest instance of the general construction above; the two-level quantum system emerges as the minimal enforcement algebra for a single competing pair of distinctions.
...
\textit{Step 1} (Hypothesis verification: $\dim \mathcal{H} \ge 3$).  By T2, the enforcement algebra contains at least $M_2(\mathbb{C})$ (a single competing pair of distinctions).  By M, at least two subsystems exist.  By $T_{\mathrm{tensor}}$ (below), the composite Hilbert space has $\dim \mathcal{H}_{AB} = \dim \mathcal{H}_A \cdot \dim \mathcal{H}_B \ge 2 \times 2 = 4 \ge 3$.

\textit{Step 2} (Budget conservation implies the frame function axioms).  A \textit{frame function} on $\mathcal{H}$ is a map $f$ from rank-one projections to $[0,1]$ satisfying: for every orthonormal basis $\{|e_i\rangle\}_{i=1}^n$ of $\mathcal{H}$, $\sum_{i=1}^n f(|e_i\rangle\langle e_i|) = 1$.  We now show that A1 forces exactly this structure.

Let $\{P_i\}_{i=1}^n$ be a complete set of orthogonal rank-one projections ($\sum_i P_i = I$), corresponding to a maximal measurement that resolves all distinctions at interface $\Gamma$.  Each $P_i$ projects onto a one-dimensional subspace representing a definite enforcement outcome---a state in which the measured distinction has a definite value.  Define $f(P_i) = \varepsilon(P_i) / C_\Gamma$, the fraction of local capacity committed to outcome $i$.  We verify the three frame-function properties:
\begin{varlist}
\item \textit{Non-negativity:} $f(P_i) \ge 0$ because enforcement costs are non-negative (Section~\ref{sec:defs}).
\item \textit{Normalization:} $\sum_i f(P_i) = \sum_i \varepsilon(P_i)/C_\Gamma = C_\Gamma/C_\Gamma = 1$.  The second equality holds because a complete measurement exhausts the local budget: each distinction at $\Gamma$ is resolved by exactly one outcome, so the costs partition the total local capacity.  (If $\sum_i \varepsilon(P_i) < C_\Gamma$, unresolved capacity would correspond to an enforceable distinction not captured by any outcome, contradicting completeness.  If $\sum_i \varepsilon(P_i) > C_\Gamma$, the measurement would require more capacity than is available, violating A1.)
\item \textit{Basis independence:} $f$ depends on the projections, not on the choice of basis.  This follows because enforcement costs are properties of the physical distinctions being maintained, not of the labels assigned to them.  A relabeling of the basis (a unitary rotation of the resolution) permutes the projections and their costs but cannot change the total.
\end{varlist}
Therefore $f$ is a frame function on $\mathcal{H}$.
...
\textit{Step 3} (Cocycle condition from algebraic consistency).  Consider three interfaces $\Gamma_i, \Gamma_j, \Gamma_k$ with overlapping enforcement regions.  At each pairwise overlap, a relabeling equivalence defines a transition automorphism $\alpha_{ij} \in \mathrm{Aut}(\mathcal{A}_\Gamma)$, which by Step~2 corresponds to a unitary conjugation: $\alpha_{ij}(X) = g_{ij} X g_{ij}^\dagger$ with $g_{ij} \in U(n_\Gamma)$ (defined up to a central phase, hence $g_{ij} \in PU(n_\Gamma)$).  At triple overlaps, the composition of relabelings must be path-independent: relabeling from $i$ to $j$ to $k$ must agree with relabeling directly from $i$ to $k$.  Algebraically, $\alpha_{ik} = \alpha_{jk} \circ \alpha_{ij}$, which translates to:
\begin{equation}
g_{ij} \cdot g_{jk} \;=\; g_{ik} \qquad \text{(cocycle condition on every triple overlap)}.
\end{equation}
This is not a condition to be verified for specific groups; it is a \emph{consequence} of the fact that automorphism composition is associative and relabeling equivalences are transitive.  The cocycle condition is the algebraic content of the statement that ``the labeling convention is arbitrary but the physical distinctions are not.''

\textit{Step 4} (Principal bundle structure).  By standard fiber bundle theory, a collection of local symmetry groups $\{G_\Gamma\}$ indexed by a topological space, together with transition functions $\{g_{ij}\}$ satisfying the cocycle condition on every triple overlap, defines a principal $G$-bundle $P \to \mathcal{M}$.  The structure group $G$ is the common automorphism group $PU(n)$ of the local algebras, and the base space $\mathcal{M}$ is the set of interfaces (given a topology in Paper~5).  The associated connection---the gauge field---arises from the requirement of parallel transport: comparing enforcement configurations at neighboring interfaces requires a path-dependent prescription, which is precisely the content of a connection 1-form.

\textit{Step 5} (Gauge group recovery).  The specific gauge group ($SU(3) \times SU(2) \times U(1)$) and its representation on matter fields are not determined at this stage; they require knowing the block structure $\{n_k\}$ of the enforcement algebra, which depends on the capacity budget (Paper~4A).  What is determined here is the \emph{mechanism}: any system of local matrix algebras with arbitrary internal labeling necessarily carries gauge structure.  The compact group $G$ is recovered from its representation category via Tannaka--Krein duality.
...
\begin{enumerate}
  \item \textit{$T_{\mathrm{Born}}$}: Budget conservation over outcomes $\to$ frame function.  (This is now a derivation rather than an identification: Step~2 of $T_{\mathrm{Born}}$ verifies non-negativity, normalization, and basis-independence explicitly from the enforcement axioms.  The only additional input is that a complete measurement exhausts the local budget---i.e., that the projections resolve all distinctions at the interface.)
...
Of these, identification~1 has been substantially reduced (from a heuristic analogy to a verified mapping with one stated premise).  Identifications~2--5 remain genuine interpretive bridges.  Each is physically motivated and consistent with A1, but none is a pure deduction from the axiom.  They represent the points where the framework's physical interpretation meets the mathematical formalism.
...
\begin{enumerate}
  ...
  \item \textbf{The identifications (Section 8.3).}  Four interpretive bridges connect enforcement-theoretic concepts to standard quantum mathematics.  (The fifth---$T_{\mathrm{Born}}$'s frame function---has been promoted from identification to derivation.)  Each remaining identification is physically motivated, but a skeptic could challenge any of them as underdetermined by A1 alone.
